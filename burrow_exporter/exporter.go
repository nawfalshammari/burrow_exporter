...
func (be *BurrowExporter) processActiveBrokers(cluster string, numberOfActiveBrokers int) {
	KafkaActiveBrokers.With(prometheus.Labels{
		"cluster":   cluster,
	}).Set(float64(numberOfActiveBrokers))
}

func (be *BurrowExporter) processCluster(cluster string) {
	
	clusterDetails, err := be.client.ClusterDetails(cluster)

	if err != nil {
		log.WithFields(log.Fields{
			"err":     err,
			"cluster": cluster,
		}).Error("error receiving cluster details. returning.")
		return
	}else{
		be.processActiveBrokers(cluster, len(clusterDetails.Cluster.Servers))
	}
